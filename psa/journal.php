...
if (isset($_GET['download_journal_id'])) {
  ...
  $filename = $file['file'] ?? '';
  $filePath = "uploads/journals/$filename";
  ...
  if ($filename && file_exists($filePath)) {
    ...
    die("Journal file not found.");
  }
}

// --- DOWNLOAD LOGIC for topics ---
if (isset($_GET['download_topic_id'])) {
  $tid = (int) $_GET['download_topic_id'];
  $topicFile = $conn->query("SELECT file FROM journal_topics WHERE id = $tid")->fetch_assoc();
  $filename = $topicFile['file'] ?? '';
  $filePath = "uploads/journals/$filename";

  if ($filename && file_exists($filePath)) {
    // Increment downloads count
    $conn->query("UPDATE journal_topics SET downloads = downloads + 1 WHERE id = $tid");
            ...
            <div class="col-md-3 mb-2">
              <select name="author" class="form-control">
                ...
                <?php while($a = $authors->fetch_assoc()): ?>
                  <option value="<?= htmlspecialchars($a['author']) ?>" <?= ($a['author'] == $authorFilter) ? 'selected' : '' ?>><?= htmlspecialchars($a['author']) ?></option>
                <?php endwhile; ?>
              </select>
            </div>
            <div class="col-md-3 mb-2">
              <select name="year" class="form-control">
                ...
                <?php while($y = $years->fetch_assoc()): ?>
                  <option value="<?= (int)$y['yr'] ?>" <?= ($y['yr'] == $yearFilter) ? 'selected' : '' ?>><?= (int)$y['yr'] ?></option>
                <?php endwhile; ?>
              </select>
            </div>
          ...
          <?php while($j = $journals->fetch_assoc()): ?>
            <div class="bg-white rounded shadow-sm mb-4 p-4 wow fadeIn" data-wow-delay="0.1s">
              ...
              ?>
                <div class="mb-2">
                  ...
                  <?php while($t = $topics->fetch_assoc()): ?>
                    <a href="journal.php?download_topic_id=<?= $t['id'] ?>" class="badge badge-info mr-1" target="_blank" rel="noopener noreferrer" title="Download <?= htmlspecialchars($t['topic']) ?> PDF">
                      <?= htmlspecialchars($t['topic']) ?> (<?= (int)$t['downloads'] ?>)
                    </a>
                  <?php endwhile; ?>
                </div>
              <?php endif; ?>
              ...
              <?php if (!empty($j['file']) && file_exists('uploads/journals/' . $j['file'])): ?>
                <a href="journal.php?download_journal_id=<?= $j['id'] ?>" class="btn btn-sm btn-outline-primary" title="Download journal PDF">
                  <i class="ti-download"></i> Download Journal
                </a>
